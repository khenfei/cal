...
public class PdfBoxPDFGenerator implements PDFGenerator {
	...
	private List<AncestorLabel> digest(
			List<JSONStringEnable> jsonObject) 
	...
	@Override
	public boolean execute(OutputStream oStream) 
			throws IOException, PDFGeneratorException {
		if(this.source == null) {
			throw new MissingData(
					new StringBuilder("Missing data detected. ")
					.append("Please supply data value with data() method. ")
					.append("Value must not be null")
					.toString());
		}
	...
	private boolean renderPage(
			...
			, List<AncestorLabel> records) throws IOException {
	...
	private PdfBoxPDFGenerator renderColumn(
			final PDDocument doc
			, final PDPage page
			, final AncestorLabel aLabel
			, final float xAxis
			, final float yAxis) throws IOException {
		...
		renderField(
			doc, page
			, aLabel.ancestor()
			, xAxis+point(23), yAxis
			, 50, VerticalAlignment.MIDDLE);
		renderField(
			doc, page
			, aLabel.number()
			, xAxis+point(53), yAxis+point(0)
			, 20, VerticalAlignment.BOTTOM);
		return this;
	}
	
	private PdfBoxPDFGenerator renderField(
			final PDDocument doc
			, final PDPage page
			, final Field field
			, final float xAxis
			...
			, final VerticalAlignment verticalAlignment) throws IOException {
	...
	private PdfBoxPDFGenerator renderCharacter(
			final PDDocument doc
			, final PDPage page
			, final List<Character> text
			, final float xAxis
			, final float yAxis
			, final float fontSize
			, final VerticalAlignment verticalAlignment) throws IOException {
		...
		final float vAlignModifier 
			= verticalAlignment == VerticalAlignment.BOTTOM? new Float(10) : 0;
